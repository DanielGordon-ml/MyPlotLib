...
class TS_visual():
    ...
    def __init__(self, df:pd.DataFrame,date_column:str, setting_dict:dict=None, theme_name:str='default') -> None:
        ...
        if self.date_column not in self.df.index.names:
    ...
    def line_plot(self, ts_column: str, display=True, save: bool = False, save_path: str = None):
        ...
        plt.plot(self.df.index, self.df[ts_column].values, label=ts_column, color=color)
        plt.xlabel('Date')
    ...
    def lines_plot(self, ts_columns: list, display=True, save: bool = False, save_path: str = None):
        ...
        for i, col in enumerate(ts_columns):
            ...
            plt.plot(self.df.index, self.df[col].values, label=col, color=color)
    ...
    def autoCorrelation_plot(self,ts_column:str,display=True, save:bool=False, save_path:str=None):
        ...
        acf_values,conf,_,_ = acf(self.df[ts_column],alpha=0.05,qstat=True)
    ...
    def partialAutoCorrelation_plot(self,ts_column:str,display=True, save:bool=False, save_path:str=None):
        ...
        pacf_values,conf= pacf(self.df[ts_column],alpha=0.05,method='ols')
    ...
    def histogram_plot(self,ts_column:str,display=True, save:bool=False, save_path:str=None,bin_size:int=100):
        ...
        '''
        plt.grid(False)
        plt.hist(self.df[ts_column],bins=bin_size)
        plt.xlabel('Value')
    ...
    def box_plot(self, ts_column: str, display=True, save: bool = False, save_path: str = None):
        ...
        self.df[ts_column].plot(kind='box')
        plt.ylabel(f'{ts_column}')
    ...
    def full_statistics_plot(self, ts_column: str, display=True, save:bool=False, save_path:str=None):
        ...
        decomposition = seasonal_decompose(self.df[ts_column], model='additive', period=3)
        ...
        ax1.plot(self.df.index, self.df[ts_column], label='Original', color=color)
        ax1.plot(self.df[ts_column].rolling(window=3).mean(), label='3-Step MA', color=color2, linewidth=1)
        ax1.plot(self.df[ts_column].rolling(window=3).std(), label='3-Step Std', color=color3, linestyle='dashed', linewidth=1)
        ax1.set_title(f'Main Time Series: {ts_column}')
        ax1.legend(loc='upper left')

        # ACF plot with confidence interval
        ax2 = plt.subplot2grid((5, 2), (1, 0))
        acf_values, conf_int = acf(self.df[ts_column], nlags=20, alpha=0.05)
        ...
        pacf_values, conf_int = pacf(self.df[ts_column], nlags=20, alpha=0.05, method='ols')
    ...
    def apply_theme(self, theme_name=None):
        ...
        theme = load_json(os.path.join('Config', f'{self.theme_name}_theme.json'))
